<template>
    <q-layout view="hHh lpR lFf">
        <q-drawer show-if-above v-model="leftDrawerOpen" side="left" bordered>
            ...
            <div v-else>
                <q-item clickable v-for="(data, dataIndex) in selectedDatas" :key="dataIndex" v-ripple
                    @click="addToText(data)">
                    <q-item-section>
                        {{ data.text }}
                        <q-item-label caption v-if="data.formula"> {{ data.formula }}</q-item-label>
                    </q-item-section>
                </q-item>
            </div>
        </q-drawer>
    ...
    computed: {
        sourceDatas() {
            ...
            templateStore.templateDataSource.categories.forEach((category) => {
                category.datas.forEach((data) => allDatas.push(data, data.color = category.color))
            })
            return allDatas;
        }
    },
    methods: {
        ...
        addToText(data) {
            ...
            if (this.sidePanelType === 'indicator') {
                this.$refs.editorPanel.runCmd('insertHTML',
